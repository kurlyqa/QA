...
class LMS_Login(unittest.TestCase):
    def setUp(self):
        # 사람처럼 보이게 하는 옵션들
        chrome_options = webdriver.ChromeOptions()
        chrome_options.add_argument('disable-gpu')  # 가속(GPU) 사용 안함
        chrome_options.add_argument('lang=ko_KR')  # 언어 설정
        self.driver = webdriver.Chrome(service=Service(ChromeDriverManager().install()), options=chrome_options)  # 크롬 드라이버 설치
        # 웹페이지 전체가 로드 될때까지 기다린다.
        self.driver.implicitly_wait(30) # 또는 self.driver.set_page_load_timeout(30)

    # 반복 되는 부분을 모듈화 한 함수
    def interact(self, by_type, name, wait_sec=1, click=True, send_keys_msg=None):
        if by_type == 'XPATH':
    ...
    def test_01_Login(self):
        ...
        # 계속 생성되는 크롬 아이콘 없애기 위해 크롬창 모두 닫기(기존에 사용하던 chrome창은 살리고 싶으면 주석처리 해도 됨)
...
if __name__ == '__main__':
    ...
    suite = unittest.TestLoader().loadTestsFromTestCase(LMS_Login)
